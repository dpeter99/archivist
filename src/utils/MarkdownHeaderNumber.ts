...
import {renderAttrs, renderHref} from "./MarkdownHeaderLink.ts";


export namespace headingNumbers{
    export class Options{
        excludeTrailingZeros:boolean = true;
        shiftHeadings:number = 0;
    }

    function headingLevel(token:Token) { return token.tag && parseInt(token.tag.substr(1, 1)); }
